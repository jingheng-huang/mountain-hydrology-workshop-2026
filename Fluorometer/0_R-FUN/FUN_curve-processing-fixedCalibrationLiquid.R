...
make_date_time = function(x){
  get_hour <- function(x, tz = "UTC") {
    # If already POSIXt, just format
    if (inherits(x, "POSIXt")) {
      return(as.numeric(format(x, "%H")))
    }
    
    # Otherwise assume character and try several formats
    # 1) try full datetime
    res <- as.POSIXct(x, format = "%Y-%m-%d %H:%M:%S", tz = tz)
    if (all(!is.na(res))) {
      return(as.numeric(format(res, "%H")))
    }
    
    # 2) try time-only; prepend dummy date
    res2 <- as.POSIXct(paste("1970-01-01", x), format = "%Y-%m-%d %H:%M:%S", tz = tz)
    if (all(!is.na(res2))) {
      return(as.numeric(format(res2, "%H")))
    }
    
    # if mixed formats: handle elementwise with tryFormats
    res3 <- strptime(x, format = "", tz = tz,
                     tryFormats = c("%Y-%m-%d %H:%M:%S", "%H:%M:%S"))
    as.numeric(format(res3, "%H"))
  }
  # 1 - make sure the manual time shift is not creating a 1 day gap:
  # tmp_tm_orig = format(x$X2, '%H:%M:%S')  # 3rd column local time
  tmp_tm_orig_h = get_hour(x$X2)  # 3rd column local time
...
# -------- Calibration -------
calibrate = function(x, start, end){
  # x = d
  print(paste0("---- Identifying calibration levels ----"))
  seq_cal = seq(1,length(x$Tracer1_mmed9))  # all points
  seq_cal = seq(start,end)                      # some points at beginning - if difficult to see the calib
  ...
  if(n_c == 1 & c1){
    ...
    # For 1 calibration point (2 times the amount)
    cs = c(98.04)
    mVs = c(m2)
    print(paste0('Using one calibration point for concetrations of ', cs, ' ppb and signal of ',round(mVs,2), ' mV'))
  }else if(n_c == 2 & c1 & c2){
